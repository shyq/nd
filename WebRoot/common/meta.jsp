<meta http-equiv="Content-Type" content="text/html;charset=utf-8"/>
<meta http-equiv="Cache-Control" content="no-store"/>
<meta http-equiv="Pragma" content="no-cache"/>
<meta http-equiv="Expires" content="0"/>
<meta name="author" content="chain" />

<link rel="stylesheet" href="${ctx}/plugin/bootstrap/css/bootstrap.css">
	
<!-- Optional theme -->
<link rel="stylesheet" href="${ctx}/plugin/bootstrap/css/bootstrap-theme.css">
<link rel="stylesheet" href="${ctx}/plugin/pagin/pagination.css">
<link rel="stylesheet" href="${ctx}/css/docs.css">
<link href="${ctx}/css/board.css" rel="stylesheet">

<!-- Latest compiled and minified JavaScript -->
<script src="${ctx}/plugin/jquery/jquery.min.1.7.js"></script>
<script src="${ctx}/plugin/bootstrap/js/bootstrap.min.js"></script>
<script src="${ctx}/plugin/pagin/jquery.pagination.js"></script>



<!-- table -->
<link rel="stylesheet" href="${ctx}/plugin/datatable/dataTables.bootstrap.css">
<link rel="stylesheet" href="${ctx}/plugin/datatable/TableTools.css">
<script src="${ctx}/plugin/datatable/jquery.dataTables.js"></script>
<script src="${ctx}/plugin/datatable/dataTables.bootstrap.js"></script>
<script src="${ctx}/plugin/datatable/TableTools.js"></script>


<script src="${ctx}/js/business/doc/jQuery.network.disk.js"></script>

